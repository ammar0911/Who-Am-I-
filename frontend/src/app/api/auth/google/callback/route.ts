...
const createCallbackResponse = (status: 'success' | 'error', error?: string) => {
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (access_token) {
            response.headers.append('Set-Cookie', `google-access-token=${access_token}; HttpOnly; Path=/; Max-Age=${tokens.expiry_date! - Date.now() / 1000}`);
        }
        if (refresh_token) {
            response.headers.append('Set-Cookie', `google-refresh-token=${refresh_token}; HttpOnly; Path=/; Max-Age=31536000`); // Expires in 1 year
        }

        return response;

    } catch (error: unknown) {
        console.error('Error exchanging code for tokens:', error instanceof Error ? error.message : String(error));
        return createCallbackResponse('error', 'Failed to authenticate with Google.');
    }
}